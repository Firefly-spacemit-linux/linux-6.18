...
#endif
{
	...
	uint8_t reg_value;
	int retry_count, ret;
	const char * driver_name =  "wdt_ctl";
	...
	for (retry_count = 0; retry_count < 100; retry_count++) {
	    ...
	    if(0 != ret) {
		dev_err(&client->dev,"====== i2c detect failed watchdog init err: 0x%x ======\n", reg_value);
	    } else {
		dev_err(&client->dev,"====== i2c detect success watchdog init ======\n");
		break;
	    }
	    msleep(10);
	}
	...
	if (0 != ret) {
	    dev_err(&client->dev,"====== i2c detect failed watchdog init ======\n");
	    goto err;
	}
...
static void pc9202_wdt_remove(struct i2c_client *client)
{

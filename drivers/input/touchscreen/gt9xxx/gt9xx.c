...
#else
		hrtimer_setup(&ts->timer, gtp_timer_handler,
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	of_property_read_u32(np, "goodix,has-pen",&pdata->has_pen);
	if (pdata->has_pen)
		dev_info(dev, "has_pen enabled\n");
...
static int gtp_init_ext_watchdog(struct i2c_client *client)
{
	...
	dev_dbg(&client->dev, "Failed init ext watchdog\n");
	return -EINVAL;
}
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	u8 buf[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};//id
	...
	/*when tp power off, but not sleep, return */
	ret = gtp_i2c_read(ts->client, buf, sizeof(buf));
	if(ret < 0 && !test_bit(SLEEP_MODE, &ts->flags)){
		dev_dbg(&ts->client->dev, "Failed read fw_info\n");
...
#endif
#if !defined(GTP_CONFIG_FB) && defined(CONFIG_PM)
		.pm		  = &gtp_pm_ops,
#endif
	},
};
